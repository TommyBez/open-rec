...
| `release-artifacts.yml` | merged-PR-to-`main` desktop release flow: patch bump desktop version, build Linux (`.AppImage`, `.deb`) and unsigned macOS (`.dmg`) artifacts on native runners, then create/update the GitHub release with stable filenames + checksums | `ubuntu-22.04`, `macos-15`, `macos-15-intel` |
